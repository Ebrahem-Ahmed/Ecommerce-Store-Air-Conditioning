...
public class PaymentService : IPaymentService
{
    ...
    public async Task<PagedResultDto<PaymentDto>> GetPaymentsByFilterAsync(PaymentFilterDto filter)
    {
        try
        {
            var query = _paymentRepository.GetAll().Where(p => !p.IsDeleted);
            ...
            if (filter.StartDate.HasValue)
                query = query.Where(p => p.ProcessedAt >= filter.StartDate.Value);

            if (filter.EndDate.HasValue)
                query = query.Where(p => p.ProcessedAt <= filter.EndDate.Value);
            ...
            if (!string.IsNullOrEmpty(filter.TransactionId))
                query = query.Where(p => p.TransactionId == filter.TransactionId);

            if (filter.OrderId.HasValue)
                query = query.Where(p => p.OrderId == filter.OrderId.Value);

            // Sorting (using System.Linq.Dynamic.Core)
            // var sortField = !string.IsNullOrEmpty(filter.SortBy) ? filter.SortBy : "ProcessedAt";
            // var sortDirection = filter.SortDirection?.ToLower() == "asc" ? "asc" : "desc";
            ...
            // Map to DTO (assuming you use AutoMapper or manual mapping)
        ...
        {
            _logger.LogError(ex, "Error getting payments by filter");
            return new PagedResultDto<PaymentDto>
            {
    ...
    public async Task<OperationResult<PaymentDto>> ProcessPaymentAsync(PaymentCreateDto dto)
    {
        try
        {
            _logger.LogInformation("Starting payment processing for Order {OrderId}, Amount {Amount}", dto.OrderId,
                dto.Amount);
            ...
            _logger.LogInformation("Created payment entity with ID {PaymentId} for Order {OrderId}", payment.Id,
                payment.OrderId);
            ...
            catch (Exception dbEx)
            {
                _logger.LogError(dbEx,
                    "Database error while saving payment for Order {OrderId}. Payment: PaymentMethod={PaymentMethod}, Amount={Amount}, TransactionId={TransactionId}",
                ...
                {
                    return OperationResult<PaymentDto>.Fail(
                        "Duplicate payment transaction detected. Please try again.");
                }
                ...
                string.Equals(dto.PaymentMethod, "COD", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    _logger.LogError(updateEx, "Failed to update COD payment status for Payment {PaymentId}",
                        payment.Id);
                }
            }
            else
            ...
            _logger.LogInformation("Payment processing completed successfully for Order {OrderId}, Payment {PaymentId}",
